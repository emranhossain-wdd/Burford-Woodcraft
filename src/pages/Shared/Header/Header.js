                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6">
                    <div className="flex justify-between items-center border-b-2 border-gray-100 py-6 md:justify-start md:space-x-10">
                        ...
                        <Popover.Group as="nav" className="hidden md:flex space-x-10 items-center">
                            <Link to="/explore" className="text-base font-medium text-gray-500 hover:text-gray-900">
                                Explore
                            </Link>
                            ...
                            {user?.email && <Popover className="relative">
                                {({ open }) => (
                                    <>
                                        <Popover.Button
                                            className={(
                                                open ? 'text-indigo-600' : 'text-indigo-600',
                                                'group bg-white rounded-md inline-flex items-center text-base font-medium focus:outline-none'
                                            )}
                                        >
                                            <UserCircleIcon
                                                className={(
                                                    open ? 'text-indigo-600' : 'text-indigo-400',
                                                    'ml-2 h-8 w-8 group-hover:text-indigo-700'
                                                )}
                                                aria-hidden="true"
                                            />
                                        </Popover.Button>
                                        ...
                                        >
                                            <Popover.Panel className="absolute z-10 left-1/2 transform -translate-x-1/2 mt-3 px-2 max-w-md sm:px-0">
                                                <div className="rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
                                                    <div className="relative grid gap-4 bg-white px-5 py-6 sm:gap-2 sm:p-6">
                                                        <Link to="/dashboard">
                                                            <div className="flex hover:bg-gray-100 rounded-lg p-3">
                                                                <PresentationChartBarIcon className="h-6 w-6 text-indigo-600" aria-hidden="true" />
                                                                <p className="ml-4">Dashboard</p>
                                                            </div>
                                                        </Link>
                                                        <div onClick={logOut} className="flex hover:bg-gray-100 rounded-lg p-3">
                                                            <LogoutIcon className="h-6 w-6 text-indigo-600" aria-hidden="true" />

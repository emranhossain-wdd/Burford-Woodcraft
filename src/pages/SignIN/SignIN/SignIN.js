        ...
        <div className="max-w-7xl px-4 sm:px-6 mx-auto py-6">
            ...
            <div className="flex justify-around items-center bg-gray-50 rounded-xl mt-8">
                <form className="flex flex-col space-y-4 mt-8" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    {/* errors will return when field validation fails  */}
                    {
                        errors.password && <span className="bg-indigo-50 text-indigo-500 rounded-md">This field is required</span>
                    }
                    {
                        signInError && <p>{signInError}</p>
                    }
                    <input className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-md p-2" type="submit" value="Sign in" />
                    <p>Already have an Account? <span className="text-indigo-600"><Link to="/signup">Please Sign up</Link></span></p>
                </form>
